  ...
  const handleStateChange = ({ status, errorMessage, info }: InitParams) => {
    ...
    if (status) {
      if (info.sources) {
        ...
        // 还原为上游逻辑：qualityList 完全由自定义源脚本注册的 sources 决定。
        // 不要在这里用展开合并塞入 bilibili —— 那会和 core/apiSource.ts 里
        // setApiSource('bilibili') 的 `qualityList = supportQuality['bilibili']`
        // 覆盖行为互相冲突，造成 qualityList 在两种状态间抖动，表现为
        // 「自定义源歌曲随机播放失败」。
        // bilibili 的质量表在用户手动选中内置源 bilibili 时由 setApiSource 正确填入。
